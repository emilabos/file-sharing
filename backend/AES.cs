...
public class AES
{
    private static readonly byte[] sbox = new byte[256] {
        // S-box (same as standard AES sbox)
    ...
    private static byte[,] KeyExpansion(byte[] key)
    {
        ...
        {
            byte[] temp = { w[0, col - 1], w[1, col - 1], w[2, col - 1], w[3, col - 1] };
            ...
            for (int row = 0; row < 4; row++)
                w[row, col] = (byte)(w[row, col - 4] ^ temp[row]);
        }
    ...
    private static void InvMixColumns(byte[,] state)
    {
        for (int c = 0; c < 4; c++)
        {
            ...
            state[0, c] = (byte)(GMul(a0, 0x0e) ^ GMul(a1, 0x0b) ^ GMul(a2, 0x0d) ^ GMul(a3, 0x09));
            state[1, c] = (byte)(GMul(a0, 0x09) ^ GMul(a1, 0x0e) ^ GMul(a2, 0x0b) ^ GMul(a3, 0x0d));
            state[2, c] = (byte)(GMul(a0, 0x0d) ^ GMul(a1, 0x09) ^ GMul(a2, 0x0e) ^ GMul(a3, 0x0b));
            state[3, c] = (byte)(GMul(a0, 0x0b) ^ GMul(a1, 0x0d) ^ GMul(a2, 0x09) ^ GMul(a3, 0x0e));
        }
    }
    ...
    private static byte GMul(byte a, int b)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            aa = (byte)((aa << 1) ^ ((aa & 0x80) != 0 ? 0x1b : 0x00));
        }
